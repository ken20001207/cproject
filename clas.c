...
if(textbox(GenUIID(0), x+20+5+w, y, 30, h, studentnumber, sizeof(studentnumber)));//从键盘输入学号（这段仅是示意 
if(textbox(GenUIID(0), x+20+5+w, y, 30, h, studentname, sizeof(studentname)));//从键盘输入学号（这段仅是示意 
...
int clas_input_score(struct Class *cp, struct Score_node *rpn, char *schoolnumber, char *score) {  //录入同学成绩
    if (clas_check_stud(cp, schoolnumber)) return 1;
    int thescore = 0, i = 0;
    while (score[i] != '\0') {
        thescore = thescore * 10 + score[i] - '0';
        i++;
    }
    while (rpn != NULL) {
...
//在之前用struct Course *getCourseByCode(char *course_code) 得到up，然后输入up-> scores作为rpn
